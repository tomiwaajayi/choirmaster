...
- Built-in markdown picker: `cm run` and `cm plan` with no path open a searchable picker owned by ChoirMaster, so users do not need shell completion to find a plan.
...
Users can also run `choirmaster plan <plan.md>` to generate the task file under `.choirmaster/tasks/` without executing it, then inspect the generated contract before running the markdown plan. The task file is still deliberately readable and executable for advanced workflows, but it is no longer part of the normal user-facing path.
...
- `cm --resume <run-id>` is a top-level resume shortcut, and blocked, paused, or interrupted runs print the exact command to continue.
...
- Interactive shell: `cm` opens a persistent prompt with slash commands, and Tab completion inside that prompt can suggest markdown references without relying on the user's shell.
- Built-in markdown picker: `cm run` and `cm plan` can ask the user to choose a markdown file inside the CLI, avoiding reliance on shell-specific autocomplete setup.
...
- `choirmaster init`
- `cm` for the persistent interactive prompt
- `choirmaster doctor`
...
- `choirmaster run` and `choirmaster plan` for ChoirMaster's built-in markdown picker
